...
export const submitContact = async (req, res) => {
    try {
        const { type, fullName, email, inquiryType, topic, priority, message } = req.body;
        ...
        if (req.file) {
            try {
                const uploadResult = await uploadToR2(req.file.path, req.file.originalname);
    ...
    `;
        try {
            await transporter.sendMail({
                from: process.env.MAIL_FROM || `"System Alert" <${process.env.SMTP_USER}>`,
